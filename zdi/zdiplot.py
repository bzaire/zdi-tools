...
def interp_data(x, y, z):
    ...
    for i in range(y.shape[0]):
        Z[iy[i], :] = N.interp(xvals, x[iy[i]], z[iy[i]])
    return Z
...
def SpecI(filename, iSort=False, save=''):
    # Read data
    ...
    if iSort:
        ...
        P.ylabel(r'Roation phase')
#        P.ylim((0,1))
    else:
        tZ = interp_data1(vrI, cycleI, I)
    ...
    P.xlabel(r'$v_\mathrm{r} (\mathrm{km/s}) $')
    P.tight_layout()
    P.colorbar(extend='both')
    if save != '':
        P.savefig(save+'.pdf')
    P.show()

def SpecHa(filename, iSort=False):
    # Read data
    cycleI, vrI, snI, I, cycleV, vrV, snV, V =  rstokes(filename)
    vmin = min([min(i) for i in vrI])
    vmax = max([max(i) for i in vrI])
    ...
    # Plot dynamic spectra
    cmap = cmocean.cm.matter  # Colormap used in plot
    P.figure(figsize=(4,3))
    ax1 = P.subplot(111)
    pc = 0.3
    if iSort:
...
def DynSpecI(filename, filename2=None, save=''):
    ...
    # Plot dynamic spectra
    fig, ax = P.subplots(1,3,figsize=(8, 3), sharex=True, sharey=True)
    pc=0.003
    ...
    ax[0].set_ylabel(r'Rotation cycle')
    ax[0].set_ylabel(r'Rotation phase')
    P.ylim((0,1))
    ax[0].set_title(r'Data')
    ax[1].set_title(r'Model')
    ax[2].set_title(r'Residuals $\times 10^{4}$')
    ax[1].set_xlabel(r'$v_\mathrm{r} (\mathrm{km/s}) $')
    if save != '':
        P.savefig(save+'.pdf')
    P.tight_layout()
    P.show()
